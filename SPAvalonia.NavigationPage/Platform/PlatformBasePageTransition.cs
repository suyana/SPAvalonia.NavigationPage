...
public abstract class PlatformBasePageTransition : IPageTransition {
    ...
    protected abstract CompositionAnimationGroup GetOrCreateEnteranceAnimation(CompositionVisual element,
    ...
    protected virtual Task RunAnimationAsync(
        ...
        if (toElement != null) {
            var animation = forward
                ? _enteranceAnimation ??=
                    GetOrCreateEnteranceAnimation(parentComposition, widthDistance, heightDistance)
                : _bringBackAnimation ??=
