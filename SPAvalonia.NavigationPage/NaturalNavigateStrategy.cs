...
public class NaturalNavigateStrategy : INavigateStrategy {
    ...
    public virtual Task<string> NavigateAsync(NavigationChain chain, string currentUri, string path, CancellationToken cancellationToken) {
        var newUrl = path;
        if (!_navigationRegistrar.TryGetNode(newUrl, out var node))
            return Task.FromResult(newUrl);

        var defaultNode = node; //.GetLastDefaultNode();

        var uri = defaultNode.Route;
    ...
    public virtual Task<string?> BackAsync(NavigationChain chain, string currentUri, CancellationToken cancellationToken) =>
        Task.FromResult<string?>(currentUri+"..");
}